...
/**
 * زنجیرهٔ فیلترهای سخت‌گیرانهٔ کریپتو — نسخهٔ ۵٫۷ (هم‌گرایی وزن‌دار، ۳۳ شاهد).
 *
...
final class Filters
{
    ...
    private $learn;
    /** @var string گروه رژیم فعال — برای ضرایب وابسته به رژیم (نسخهٔ ۵٫۷) */
    private $curRegime = '';
    ...
    /** ضریب اثرگذار فیلتر — ۰٫۰ یعنی غیرفعال. رژیم فعال مقدم است (key@regime). */
    private function mult(string $key): float
    {
        if ($this->learn === null) {
            return 1.0;
        }
        $disabled = (array)($this->learn['disabled'] ?? []);
        if (in_array($key, $disabled, true)) {
            return 0.0;
        }
        $rg = $this->curRegime;
        if ($rg !== '' && in_array($key . '@' . $rg, $disabled, true)) {
            return 0.0;
        }
        $m = (array)($this->learn['multipliers'] ?? []);
        if ($rg !== '' && isset($m[$key . '@' . $rg])) {
            $v = (float)$m[$key . '@' . $rg];
        } else {
            $v = (float)($m[$key] ?? 1.0);
        }
        return min(3.0, max(0.05, $v));
    }
    ...
    public function evaluate(array $ctx): array
    {
        ...
        // نسخهٔ ۵٫۷: ضرایب آموخته‌شدهٔ همان رژیم اعمال می‌شوند (روند/رِنج/پرنوسان)
        $this->curRegime = LearningEngine::regimeGroup($regime);
        ...
        /* ── ۳۲) فشردگی بولینگر (Squeeze) — انباشت انرژی پیش از شکست ── */
        $bbw = $ctx['bb_width_pct'] ?? null; // صدک رتبهٔ عرض باند (۰-۱۰۰)
        if ($bbw !== null) {
            $bbw = (float)$bbw;
            if ($bbw <= 18.0) {
                $sqSide = $ema9 >= $ema21 ? 'BUY' : 'SELL';
                $f = $this->f('bb_squeeze', 'فشردگی بولینگر (Squeeze)', true, 0.75, $sqSide, 0.8,
                    'عرض باند در صدک ' . round($bbw) . ' — فشردگی انرژی؛ جهت شکست از میکروترند');
                $vote($f, 0.8);
            } elseif ($bbw >= 82.0) {
                $f = $this->f('bb_squeeze', 'فشردگی بولینگر (Squeeze)', false, 0.3, 'NEUTRAL', 0.8,
                    'عرض باند در صدک ' . round($bbw) . ' — انبساط افراطی؛ خطر دمیدگی');
            } else {
                $f = $this->f('bb_squeeze', 'فشردگی بولینگر (Squeeze)', true, 0.6, 'NEUTRAL', 0.8,
                    'عرض باند در صدک ' . round($bbw) . ' — نرمال');
            }
        } else {
            $f = $this->f('bb_squeeze', 'فشردگی بولینگر (Squeeze)', true, 0.5, 'NEUTRAL', 0.8, 'دادهٔ کافی نیست — عبور خنثی.');
        }
        $filters[] = $f;

        /* ── ۳۳) جهت‌یاب DI — تسلط خریداران/فروشندگان ─────────────── */
        $plusDi = $ctx['plus_di'] !== null ? (float)$ctx['plus_di'] : null;
        $minusDi = $ctx['minus_di'] !== null ? (float)$ctx['minus_di'] : null;
        if ($plusDi !== null && $minusDi !== null) {
            $diSpread = $plusDi - $minusDi;
            if ($diSpread >= 8.0) {
                $f = $this->f('di_spread', 'جهت‌یاب DI (+DI/−DI)', true, 0.8, 'BUY', 0.9,
                    '+DI ' . round($plusDi, 1) . ' در برابر −DI ' . round($minusDi, 1) . ' — تسلط خریداران');
                $vote($f, 0.9);
            } elseif ($diSpread <= -8.0) {
                $f = $this->f('di_spread', 'جهت‌یاب DI (+DI/−DI)', true, 0.8, 'SELL', 0.9,
                    '−DI ' . round($minusDi, 1) . ' در برابر +DI ' . round($plusDi, 1) . ' — تسلط فروشندگان');
                $vote($f, 0.9);
            } else {
                $f = $this->f('di_spread', 'جهت‌یاب DI (+DI/−DI)', true, 0.5, 'NEUTRAL', 0.9,
                    'تفاضل DI: ' . round($diSpread, 1) . ' — تعادل نسبی');
            }
        } else {
            $f = $this->f('di_spread', 'جهت‌یاب DI (+DI/−DI)', true, 0.5, 'NEUTRAL', 0.9, 'دادهٔ DI کافی نیست — عبور خنثی.');
        }
        $filters[] = $f;

        /* ── ۳۴) ریسک خبری آن‌چین — لایهٔ هفتم (نسخهٔ ۵٫۷) ───────────── */
        $sent = $ctx['sentiment'] ?? null;
        if (is_array($sent) && !empty($sent['ok']) && isset($sent['score'])) {
            $smScore = (float)$sent['score'];
            $smLabel = (string)($sent['label'] ?? '');
            $smTop = '';
            if (!empty($sent['news']['top'][0]['title'])) {
                $smTop = ' · سرخطی: ' . mb_substr((string)$sent['news']['top'][0]['title'], 0, 46);
            }
            if ($smScore <= -0.45) {
                // بحران خبری — ورود تازه متوقف؛ جهت‌دهی نمی‌کند (NEUTRAL)
                $f = $this->f('news_risk', 'ریسک خبری آن‌چین', false, 0.35, 'NEUTRAL', 0.7,
                    'سنتیمنت: ' . $smLabel . ' (' . round($smScore, 2) . ') — فشار خبری شدید' . $smTop);
            } elseif ($smScore >= 0.6) {
                $f = $this->f('news_risk', 'ریسک خبری آن‌چین', true, 0.6, 'NEUTRAL', 0.7,
                    'سنتیمنت: ' . $smLabel . ' (' . round($smScore, 2) . ') — هیجان شدید؛ حجم محافظه‌کارانه' . $smTop);
            } else {
                $f = $this->f('news_risk', 'ریسک خبری آن‌چین', true, 0.55, 'NEUTRAL', 0.7,
                    'سنتیمنت: ' . $smLabel . ' (' . round($smScore, 2) . ') — بدون مانع خبری');
            }
        } else {
            $f = $this->f('news_risk', 'ریسک خبری آن‌چین', true, 0.5, 'NEUTRAL', 0.7, 'دادهٔ سنتیمنت در دسترس نیست — عبور خنثی.');
        }
        $filters[] = $f;
