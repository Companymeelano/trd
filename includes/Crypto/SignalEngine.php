...
final class SignalEngine
{
    ...
    public function scanMarket(?int $limit = null): array
    {
        ...
        try {
            if (!isset($this->cfg['enable_sentiment']) || !empty($this->cfg['enable_sentiment'])) {
                // لایهٔ هفتم: نبض سنتیمنت آن‌چین برای کل اسکن (نسخهٔ ۵٫۷)
                $extra['sentiment'] = Sentiment::sharedPulse();
            }
        } catch (Throwable $e) { /* نبود سنتیمنت = عبور خنثی فیلتر ۳۴ */ }
        try {
    ...
    public function analyzeSymbol(string $symbol, ?array $ticker = null, ?array $btc = null, ?array $breadth = null, array $extra = []): array
    {
        ...
        if (!empty($extra['sentiment']) && is_array($extra['sentiment'])) {
            // لایهٔ هفتم — نبض سنتیمنت آن‌چین (نسخهٔ ۵٫۷)
            $ctx['sentiment'] = $extra['sentiment'];
        }
        ...
        $needPassed = $this->needFiltersPassed($eval['total']);
        $minTech = (float)($this->cfg['min_tech_score'] ?? 62);
        if ($eval['passed'] < $needPassed || $eval['tech_score'] < $minTech) {
            // نسخهٔ ۵٫۷: ژورنال فرصت‌های نزدیک — کاندیداهای نزدیک به آستانه هم
            // ثبت و داوری می‌شوند تا موتور یادگیری، هزینهٔ آستانه‌ها را ببیند
            if ($eval['tech_score'] >= $minTech - 6 && $eval['passed'] >= $needPassed - 2) {
                $this->persistNearMiss($base, $eval, $ctx, $regime);
            }
    ...
    private function inCooldown(string $symbol, string $side): bool
    {
        ...
        try {
            ...
            $row = $this->db->selectOne(
                "SELECT id FROM {$t} WHERE symbol = ? AND side = ? AND status <> 'near_miss' AND created_at >= ? LIMIT 1",
                [$symbol, $side, $cutoff]
            );
            return $row !== null;
        } catch (Throwable $e) {
    ...
    /**
     * ثبت فرصت نزدیک (نسخهٔ ۵٫۷): کاندیدایی که از آستانه جا ماند ولی نزدیک بود.
     * با داوری ردیاب، سوخت یادگیری برای کالیبره‌کردن آستانه‌ها می‌شود.
     */
    private function persistNearMiss(array $base, array $eval, array $ctx, array $regimeInfo): void
    {
        if ($this->db === null) {
            return;
        }
        try {
            if (!(bool)Config::get('learning.near_miss', true) || !$this->db->tableExists('signals')) {
                return;
            }
            $symbol = (string)$base['symbol'];
            $side = (string)$eval['side'];
            $t = $this->db->table('signals');
            $cutoff = date('Y-m-d H:i:s', time() - 6 * 3600); // ضد اسپم: هر نماد/جهت هر ۶ ساعت یک‌بار
            $dup = $this->db->selectOne(
                "SELECT id FROM {$t} WHERE symbol = ? AND side = ? AND status = 'near_miss' AND created_at >= ? LIMIT 1",
                [$symbol, $side, $cutoff]
            );
            if ($dup !== null) {
            ...
            $plan = $this->risk->plan($side, (float)$ctx['price'], (float)$ctx['atr'], (float)$eval['tech_score'], [
                'swing_low' => $ctx['swing_low'],
                'swing_high' => $ctx['swing_high'],
                'sizing_factor' => $regimeInfo['sizing_factor'],
            ]);
            $this->db->insert('signals', [
                ...
                'tier' => 'NM', // فرصت نزدیک — هرگز در آمار درجات رسمی نمی‌آید
                'regime' => (string)($base['regime'] ?? ''),
                'confidence' => (float)$eval['tech_score'],
                'tech_score' => (float)$eval['tech_score'],
                'ai_score' => 0,
                'combined_score' => (float)$eval['tech_score'],
                'mtf_score' => 0,
                'entry_price' => (float)$plan['entry'],
                'stop_loss' => (float)$plan['stop_loss'],
                'take_profit_1' => (float)$plan['take_profit_1'],
                'take_profit_2' => (float)$plan['take_profit_2'],
                'take_profit_3' => (float)$plan['take_profit_3'],
                'risk_reward' => (float)$plan['risk_reward_2'],
                'position_pct' => (float)$plan['position_percent'],
                'invalidation' => (string)($plan['invalidation'] ?? ''),
                'filters_passed' => (int)$eval['passed'],
                'filters_total' => (int)$eval['total'],
                'filters_json' => json_encode($eval['filters'], JSON_UNESCAPED_UNICODE),
                'mtf_json' => null,
                'ai_json' => '[]',
                'status' => 'near_miss',
                'created_at' => date('Y-m-d H:i:s'),
            ...
            Logger::write('crypto', 'ثبت فرصت نزدیک ناموفق: ' . $e->getMessage(), 'warning');
        }
    }
