...
(function (window, document) {
        ...
        try {
            ...
            if (inner && Array.isArray(data.top) && data.top.length) {
                const items = data.top.map(function (t) {
                    ...
                    return '<span class="tick" data-sym="' + M.escapeHtml(t.symbol || t.base) + '"><b class="mono">' + M.escapeHtml(t.base) + '</b> ' +
                        '<span class="tick__price" data-price="' + price(t.price) + '">' + price(t.price) + '</span> ' +
                        '<span class="tick__chg ' + (up ? 'up' : 'down') + '">' + (up ? '▲' : '▼') + ' ' + M.toFa(Math.abs(Math.round(t.change_pct * 10) / 10)) + '٪</span>' +
                        sparkline(t.spark, 64, 20) + '</span>';
                }).join('');
                inner.innerHTML = items + items; // برای اسکرول بی‌درز
                initLiveWs(data.top); // دادهٔ لحظه‌ای وب‌سوکت (نسخهٔ ۵٫۷)
            }
            // لایهٔ هفتم — سنتیمنت آن‌چین (نسخهٔ ۵٫۷)
    ...
    function renderLearning(st) {
        ...
        if (box) {
            const REGIME_BADGE = { trend: 'روند', range: 'رِنج', volatile: 'پرنوسان' };
            const rows = (st.filters || []).map(function (f) {
                ...
                const dir = f.directional
                    ? '<span class="mono" style="color:' + corrColor + '">' + M.toFa(corr) + '٪</span>'
                        + (f.val_correctness !== null && f.val_correctness !== undefined
                            ? ' <i class="fa-solid fa-vial" style="font-size:9px;color:' + (f.val_correctness >= 0.45 ? '#34d399' : '#fb7185') + '" title="آزمون walk-forward: ' + Math.round(f.val_correctness * 100) + '٪ با ' + M.toFa(f.val_n) + ' نمونه"></i>' : '')
                ...
                const rg = f.regime && REGIME_BADGE[f.regime]
                    ? ' <span class="chip chip--pending" style="font-size:9px;padding:1px 6px">' + REGIME_BADGE[f.regime] + '</span>' : '';
                return '<tr>'
                    + '<td style="padding:6px 8px">' + M.escapeHtml(f.label) + rg + manual + '</td>'
            ...
            box.innerHTML = (st.filters || []).length
                ? '<div class="table-wrap"><table class="data" style="width:100%;font-size:12px"><thead><tr>'
                    + '<th style="text-align:right">فیلتر</th><th>نمونه</th><th>درست‌بودن (🧪=آزمون تازه)</th><th>میانگین R</th><th>ضریب فعلی</th>'
        ...
        if (ml) {
            ml.innerHTML = (st.filters || []).filter(function (f) { return !f.regime; }).map(function (f) {
        ...
        if (ev) {
            const typeMap = { adapt: 'تطبیق وزن', quarantine: 'قرنطینه', recover: 'بازسازی', reset: 'بازنشانی', hold: 'تعویق walk-forward' };
            ev.innerHTML = (st.events || []).map(function (e) {
                const color = e.type === 'quarantine' ? '#fb7185' : (e.type === 'recover' ? '#34d399' : (e.type === 'hold' ? '#fbbf24' : '#a78bfa'));
    ...
    function renderSentiment(sm) {
        const v = el('sent_value'); const lbl = el('sent_label'); const fill = el('sent_fill');
        if (!v) { return; }
        const pct = Math.round(((sm.score || 0) + 1) * 50); // −1..+1 → 0..100
        if (sm.ok) {
            v.textContent = (sm.score >= 0 ? '+' : '−') + M.toFa(Math.abs(Math.round((sm.score || 0) * 100)));
            v.style.color = (sm.score <= -0.2) ? '#fb7185' : ((sm.score >= 0.2) ? '#34d399' : '#fbbf24');
        } else {
            v.textContent = '—';
        }
        if (lbl) { lbl.textContent = sm.ok ? (sm.label || '') : 'داده در دسترس نیست'; }
        if (fill) { fill.style.width = Math.max(2, Math.min(100, pct)) + '%'; }

        const fgEl = el('sent_fg');
        if (fgEl) {
            fgEl.textContent = sm.fg ? M.toFa(sm.fg.value) + '/۱۰۰' : '—';
            const fgSub = el('sent_fg_sub');
            if (fgSub) { fgSub.textContent = sm.fg ? (sm.fg.label || '') : 'منبع در دسترس نیست'; }
        }
        const mkEl = el('sent_market');
        if (mkEl) {
            const m = sm.market_24h;
            mkEl.textContent = (m === null || m === undefined) ? '—' : ((m >= 0 ? '+' : '−') + M.toFa(Math.abs(Math.round(m * 100) / 100)) + '٪');
            mkEl.style.color = (m || 0) >= 0 ? '#34d399' : '#fb7185';
            const mkSub = el('sent_market_sub');
            if (mkSub) { mkSub.textContent = sm.sources && sm.sources.market ? 'تغییر ارزش کل بازار (CoinGecko)' : 'منبع در دسترس نیست'; }
        }
        const nwEl = el('sent_news');
        if (nwEl) {
            const nw = sm.news || {};
            nwEl.textContent = (nw.weighted === null || nw.weighted === undefined) ? '—' : ((nw.weighted >= 0 ? '+' : '−') + M.toFa(Math.abs(Math.round(nw.weighted * 100))));
            nwEl.style.color = (nw.weighted || 0) <= 0 ? '#fb7185' : '#34d399';
            const nwSub = el('sent_news_sub');
            if (nwSub) {
                nwSub.textContent = M.toFa(nw.count || 0) + ' سرخطی از ' + M.toFa(nw.feeds_ok || 0) + ' فید · پنجرهٔ ۲۴ ساعت';
            }
        }
        const atEl = el('sent_at');
        if (atEl) { atEl.textContent = (sm.cached ? 'کش · ' : 'تازه · ') + M.escapeHtml(sm.at || ''); }

        const hl = el('sent_headlines');
        if (hl) {
            const top = (sm.news && sm.news.top) || [];
            hl.innerHTML = top.length ? top.map(function (t) {
                const neg = (t.weight || 0) < 0;
                return '<div class="signal-row" style="padding:8px 10px">' +
                    '<span class="badge ' + (neg ? 'badge--bad' : 'badge--ok') + '">' + (neg ? '▼ ریسک' : '▲ خوش‌بینی') + ' ' + M.toFa(Math.abs(t.weight || 0)) + '</span>' +
                    '<span style="margin-inline-start:8px">' + M.escapeHtml(t.title || '') + '</span></div>';
            }).join('') : '<span class="dim" style="font-size:12px">سرخطی مهمی در پنجرهٔ ۲۴ ساعت وزن نگرفت.</span>';
        }
    }

    async function loadSentiment(force) {
        try {
            const d = await M.api('api/sentiment.php', { action: force ? 'refresh' : 'status' });
            if (d.ok !== undefined) { renderSentiment(d); }
        } catch (err) { /* کارت خنثی می‌ماند */ }
    }

    /* ── دادهٔ لحظه‌ای وب‌سوکت بایننس (نسخهٔ ۵٫۷) ═════════════════════ */

    const ws = { sock: null, symbols: [], retries: 0, alive: false, lastMsg: 0, rate: 0, rateTimer: null };

    function wsBadge(text, cls) {
        const b = el('ws_state');
        if (b) { b.textContent = text; b.className = 'badge' + (cls ? ' ' + cls : ''); }
    }

    function initLiveWs(tickers) {
        const symbols = (tickers || []).slice(0, 18).map(function (t) { return (t.symbol || t.base) + ''; });
        if (!symbols.length) { return; }
        if (symbols.indexOf('BTCUSDT') === -1) { symbols.unshift('BTCUSDT'); }
        const key = symbols.slice().sort().join(',');
        if (ws.symbols.length && ws.symbols.slice().sort().join(',') === key) { return; } // بدون بازسازی
        ws.symbols = symbols;
        wsConnect();
    }

    function wsConnect() {
        if (!('WebSocket' in window)) { wsBadge('🛰️ وب‌سوکت: پشتیبانی نمی‌شود', 'badge--bad'); return; }
        try { if (ws.sock) { ws.sock.onclose = null; ws.sock.close(); } } catch (e) { /* بسته بود */ }
        const streams = ws.symbols.map(function (s) { return s.toLowerCase() + '@ticker'; }).join('/');
        const url = 'wss://stream.binance.com:9443/stream?streams=' + streams;
        wsBadge('🛰️ وب‌سوکت: در حال اتصال…', '');
        let sock;
        try { sock = new WebSocket(url); } catch (e) { wsBadge('🛰️ وب‌سوکت: مسدود', 'badge--bad'); return; }
        ws.sock = sock;
        sock.onopen = function () {
            ws.alive = true; ws.retries = 0; ws.lastMsg = Date.now();
            wsBadge('🛰️ وب‌سوکت: زنده 🟢', 'badge--ok');
            if (ws.rateTimer) { clearInterval(ws.rateTimer); }
            ws.rateTimer = setInterval(function () {
                if (!ws.alive) { return; }
                const idle = (Date.now() - ws.lastMsg) / 1000;
                if (idle > 75) { // بی‌خبری طولانی = اتصال نیمه‌مرده
                    try { sock.close(); } catch (e) { /* بازاتصال */ }
                } else {
                    wsBadge('🛰️ وب‌سوکت: زنده · ' + M.toFa(ws.rate) + ' پیام/ثانیه', 'badge--ok');
                    ws.rate = 0;
                }
            }, 1000);
        };
        sock.onmessage = function (ev) {
            ws.lastMsg = Date.now(); ws.rate++;
            let d = null;
            try { d = JSON.parse(ev.data); } catch (e) { return; }
            const t = d && d.data ? d.data : d;
            if (!t || !t.s) { return; }
            applyTick(t.s, parseFloat(t.c), parseFloat(t.P));
        };
        sock.onclose = function () {
            ws.alive = false;
            if (ws.rateTimer) { clearInterval(ws.rateTimer); ws.rateTimer = null; }
            if (ws.retries < 4) {
                ws.retries++;
                wsBadge('🛰️ وب‌سوکت: بازاتصال ' + M.toFa(ws.retries) + '/۴…', '');
                setTimeout(wsConnect, Math.min(15000, 1200 * ws.retries * ws.retries));
            } else {
                wsBadge('🛰️ وب‌سوکت: قطع — پولینگ فعال', 'badge--bad');
            }
        };
        sock.onerror = function () { try { sock.close(); } catch (e) { /* onclose می‌گیرد */ } };
    }

    function applyTick(symbol, last, chgPct) {
        if (!isFinite(last)) { return; }
        // قیمت لحظه‌ای بیت‌کوین در استات لنگر
        if (symbol === 'BTCUSDT') {
            const bp = el('btc_price');
            if (bp && bp.textContent !== '—') { bp.textContent = price(last); }
        }
        // تیک‌های نوار متحرک
        const nodes = document.querySelectorAll('.tick[data-sym="' + symbol + '"]');
        for (let i = 0; i < nodes.length; i++) {
            const n = nodes[i];
            const pEl = n.querySelector('.tick__price');
            const cEl = n.querySelector('.tick__chg');
            if (pEl) {
                const prev = parseFloat(pEl.getAttribute('data-price')) || last;
                pEl.setAttribute('data-price', String(last));
                pEl.textContent = price(last);
                if (last !== prev) {
                    pEl.style.color = last > prev ? '#34d399' : (last < prev ? '#fb7185' : '');
                    setTimeout(function (pe) { return function () { pe.style.color = ''; }; }(pEl), 450);
                }
            }
            if (cEl && isFinite(chgPct)) {
                const up = chgPct >= 0;
                cEl.className = 'tick__chg ' + (up ? 'up' : 'down');
                cEl.textContent = (up ? '▲' : '▼') + ' ' + M.toFa(Math.abs(Math.round(chgPct * 10) / 10)) + '٪';
            }
        }
    }
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        loadTracker();
        loadLearning();
        loadSentiment(false); // لایهٔ هفتم — نسخهٔ ۵٫۷
        const sentBtn = el('btn_sent_refresh');
        if (sentBtn) {
            sentBtn.addEventListener('click', function () {
                setBusy(sentBtn, true, '…');
                loadSentiment(true).then(function () { setBusy(sentBtn, false); });
            });
        }
        setInterval(loadPulse, 60000); // پالس بازار هر دقیقه (fallback وب‌سوکت)
        setInterval(loadTracker, 5 * 60000); // آمار ردیاب هر ۵ دقیقه
        setInterval(function () { if (!ws.alive) { loadSentiment(false); } }, 5 * 60000); // سنتیمنت هر ۵ دقیقه
        M.refreshSystemStatus();
    });
}(window, document));
