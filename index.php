...
if ($dbOk && $db->tableExists('signals')) {
    $row = $db->selectOne("SELECT COUNT(*) AS total, SUM(CASE WHEN side='BUY' THEN 1 ELSE 0 END) AS buys,
            ...
            FROM " . $db->table('signals') . " WHERE status <> 'near_miss'");
    $stats = [
    ...
    $recent = $db->select('SELECT * FROM ' . $db->table('signals') . " WHERE status <> 'near_miss' ORDER BY id DESC LIMIT 20");
}
...
<div class="card-3d section" id="pulse_section">
    <div class="section__head">
        ...
        <div style="display:flex;gap:6px;flex-wrap:wrap;align-items:center">
            <span id="ws_state" class="badge" title="دادهٔ لحظه‌ای وب‌سوکت بایننس">🛰️ وب‌سوکت: خاموش</span>
    ...
    <div class="grid grid--3" style="margin-bottom:14px">
        ...
        <div class="stat stat--indigo">
            <div class="stat__label"><i class="fa-solid fa-satellite-dish"></i> سنتیمنت آن‌چین (لایهٔ ۷)</div>
            <div class="stat__value" id="sent_value">—</div>
            <div class="stat__sub"><div class="fg-gauge"><div id="sent_fill" class="fg-gauge__fill" style="width:50%;background:linear-gradient(90deg,#fb7185,#fbbf24,#34d399)"></div></div><span id="sent_label">—</span></div>
        </div>
    </div>
...
<div class="card-3d section" id="sentiment_section">
    <div class="section__head">
        <h2 class="section__title"><i class="fa-solid fa-tower-broadcast"></i> لایهٔ هفتم — سنتیمنت آن‌چین و ریسک خبری</h2>
        <div style="display:flex;gap:6px;align-items:center">
            <span id="sent_at" class="badge">—</span>
            <button class="btn btn--indigo btn--sm" id="btn_sent_refresh"><i class="fa-solid fa-rotate"></i> به‌روزرسانی</button>
        </div>
    </div>
    <div class="grid grid--3" style="margin-bottom:12px">
        <div class="stat stat--rose">
            <div class="stat__label"><i class="fa-solid fa-face-grimace"></i> ترس و طمع (خلاف‌گردش)</div>
            <div class="stat__value" id="sent_fg">—</div>
            <div class="stat__sub" id="sent_fg_sub">—</div>
        </div>
        <div class="stat stat--gold">
            <div class="stat__label"><i class="fa-solid fa-globe"></i> مومنتوم کل بازار ۲۴س</div>
            <div class="stat__value" id="sent_market">—</div>
            <div class="stat__sub" id="sent_market_sub">ارزش کل بازار کریپتو</div>
        </div>
        <div class="stat stat--indigo">
            <div class="stat__label"><i class="fa-solid fa-newspaper"></i> وزن اخبار آن‌چین</div>
            <div class="stat__value" id="sent_news">—</div>
            <div class="stat__sub" id="sent_news_sub">—</div>
        </div>
    </div>
    <div id="sent_headlines" style="display:flex;flex-direction:column;gap:6px"></div>
</div>
